...
fn joy_glfw_create_window(
    ...
    if ptr.is_null() {
        ...
        unsafe {
            glfw_ffi::glfwSetWindowCloseCallback(ptr, Some(window_close_callback));
            glfw_ffi::glfwSetKeyCallback(ptr, Some(key_callback));
            glfw_ffi::glfwSetCursorPosCallback(ptr, Some(cursor_move_callback));
        };
...
static mut KEY_CALLBACK: Option<Ref<()>> = None;
static mut KEY_CALLBACK_ENV: Option<Env> = None;

#[no_mangle]
pub extern "C" fn key_callback(_window: *mut GLFWwindow, key: i32, scancode: i32, action: i32, mods: i32) {
    unsafe {
        if let Some(ref callback) = crate::KEY_CALLBACK {
            let func: JsFunction = crate::KEY_CALLBACK_ENV
                .unwrap()
                .get_reference_value(&callback)
                .unwrap();
            func.call4(key, scancode, action, mods).unwrap()
        }
    }
}

#[napi(ts_args_type = "window: ExternalObject<any>, callback: (key: number, scancode: number, action: number, mods: number) => void")]
fn joy_glfw_set_key_callback(
    env: Env,
    _window: External<*mut GLFWwindow>,
    callback: JsFunction,
) {
    let reference = env.create_reference(callback).unwrap();
    unsafe {
        crate::KEY_CALLBACK = Some(reference);
        crate::KEY_CALLBACK_ENV = Some(env);
    }
}

static mut CURSOR_MOVE_CALLBACK: Option<Ref<()>> = None;
static mut CURSOR_MOVE_CALLBACK_ENV: Option<Env> = None;

#[no_mangle]
pub extern "C" fn cursor_move_callback(_window: *mut GLFWwindow, xpos: f64, ypos: f64) {
    unsafe {
        if let Some(ref callback) = crate::CURSOR_MOVE_CALLBACK {
            let func: JsFunction = crate::CURSOR_MOVE_CALLBACK_ENV
                .unwrap()
                .get_reference_value(&callback)
                .unwrap();
            func.call2(xpos, ypos).unwrap()
        }
    }
}

#[napi(ts_args_type = "window: ExternalObject<any>, callback: (xpos: number, ypos: number) => void")]
fn joy_glfw_set_cursor_move_callback(
    env: Env,
    _window: External<*mut GLFWwindow>,
    callback: JsFunction,
) {
    let reference = env.create_reference(callback).unwrap();
    unsafe {
        crate::CURSOR_MOVE_CALLBACK = Some(reference);
        crate::CURSOR_MOVE_CALLBACK_ENV = Some(env);
    }
}

#[napi]
fn joy_glfw_set_cursor_mode(window: External<*mut GLFWwindow>, mode: i32) {
    unsafe {
        glfw_ffi::glfwSetInputMode(*window.as_ref(), glfw_ffi::CURSOR, mode)
    }
}